    ...
    <body>
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            if (isset($_POST['user_id']) && !empty($_POST['user_id']) && isset($_POST['old_password']) && !empty($_POST['old_password']) && isset($_POST['new_password']) && !empty($_POST['new_password'])
                ...
                $userResult = mysqli_query($con, "Select * from `member` WHERE (`id` = " . $_POST['user_id'] . " AND `password` = '" . md5($_POST['old_password']) . "')");
                if ($userResult->num_rows > 0) {
                    $result = mysqli_query($con, "UPDATE `member` SET `password` = MD5('" . $_POST['new_password'] . "') WHERE (`id` = " . $_POST['user_id'] . " AND `password` = '" . md5($_POST['old_password']) . "')");
                    if (!$result) {
            ...
            if (!empty($user)) {
                ...
                <div id="edit_user" class="box-content">
                    <h1>Xin chào "<?= $user['username'] ?>". Bạn đang thay đổi mật khẩu</h1>
